...
export class ActionScanTableComponent {
  ...
  public ngOnInit(): void {
  	...
  	this.addNewScanStateMachine = new StateMachine<ScanTableAggregate>(ScanTableAggregate, stages, this, true, { Index: UserInterfaceEventIds.reset, FunctionReference: this.addNewScan_reset });
  }
  ...
  private async addNewScan_reset(): Promise<number> {
  	await WaitEventAsync(UserInterfaceEventIds.reset, this.localEventBus);
  ...
  private async addNewScan_waitForBarcodeInputChange(): Promise<number> {
		...
		for (;;) {
  		const res = await Promise.race([
  			WaitEventAsync(UserInterfaceEventIds.barcodeInputChanged, this.localEventBus),
  ...
  private async addNewScan_materialSelection(): Promise<number> {
  	const waitForOne = [
  		WaitEventAsync(UserInterfaceEventIds.materialRelationSelected, this.localEventBus),
  		WaitEventAsync(UserInterfaceEventIds.materialRelationUnSelected, this.localEventBus)
  	];

  	const result = await Promise.race(waitForOne);

  	if (result == UserInterfaceEventIds.materialRelationSelected)
  ...
  private async addNewScan_materialSelected(): Promise<number> {
  	...
  	const index = await Promise.race([
  		WaitEventAsync(UserInterfaceEventIds.materialRelationUnSelected, this.localEventBus),
  		WaitEventAsync(UserInterfaceEventIds.materialRelationConfirmed, this.localEventBus)
  	]);

  	if (index == UserInterfaceEventIds.materialRelationUnSelected) {
  		this.ButtonConfirmMaterialRelationDisabled = true;
  		return StateRoutines.MaterialRelation_WaitForSelection;
  	}
  	else if (index == UserInterfaceEventIds.materialRelationConfirmed)
  ...
  private async addNewScan_waitForSaveAndSave(): Promise<number> {
  	await WaitEventAsync(UserInterfaceEventIds.saveScanButtonClicked, this.localEventBus);
